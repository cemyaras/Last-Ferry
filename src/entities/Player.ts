...
export class Player {
 ...
 constructor(scene:Phaser.Scene,private environment:Partial<PlayerEnvironment>={}){
  this.x=environment.startX??PIER.startX;this.y=environment.startY??PIER.walkY;this.facing=environment.facing??-1;
 ...
 update(dt:number){
  ...
  if(this.boardingTarget){
   ...
   const next=moveOnPromenade(this.x,this.y,this.velocity*dt,this.velocityY*dt,this.environment.obstacles,this.environment.bounds);
  ...
  this.reflection.setTexture(`traveller-${frame}`).setPosition(this.x,this.y+4).setFlipX(this.facing<0);this.shadow.setPosition(this.x+(this.x-(this.environment.lightOrigin?.(this.x)??nearestLamp(this.x).x))*light*.1,this.y+1);
 }
}
