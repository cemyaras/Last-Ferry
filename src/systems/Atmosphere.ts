...
export class Atmosphere{
 ...
 constructor(scene:Phaser.Scene){
  ...
  texture(scene,'haze',PIER.width,170,c=>{c.save();c.scale(1,.12);glow(c,790,710,680,'#afbbb11a');glow(c,1780,710,580,'#afbbb111');c.restore();});
  this.haze=scene.add.image(-110,278,'haze').setOrigin(0).setDepth(5).setScrollFactor(.4);
  texture(scene,'lighting',PIER.width,720,c=>{
   ...
   glow(c,150,474,147,'#d4b57212');
  });
  scene.add.image(0,0,'lighting').setOrigin(0).setDepth(25);
  ...
  texture(scene,'reflections',PIER.width,720,c=>{
   ...
   for(let i=0;i<150;i++){const x=30+r()*267,y=525+r()*115;line(c,x,y,x+4+r()*28,y,`rgba(190,161,99,${(1-(y-525)/130)*.12})`,1+r());}
   // Long softly broken shadows from the railing.
   for(let x=413;x<PIER.width;x+=48)polygon(c,[[x,519],[x+2,519],[x+(x-780)*.16,603],[x+(x-780)*.16-3,603]],'#0314210d');
  });
  scene.add.image(0,0,'reflections').setOrigin(0).setDepth(12);
  ...
  for(let i=0;i<48;i++)this.splashes.push({x:r()*PIER.width,y:530+r()*158,phase:r()*6});
